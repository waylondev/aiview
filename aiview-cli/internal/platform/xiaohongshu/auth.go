...
// AuthStore manages Xiaohongshu cookie persistence.
type AuthStore struct {
	dir  string
	file string
}

// credential holds Xiaohongshu authentication data on disk.
type credential struct {
	Cookie string `json:"cookie"`
}

// NewAuthStore creates a new auth store for Xiaohongshu.
func NewAuthStore() *AuthStore {
	home, _ := os.UserHomeDir()
	dir := filepath.Join(home, ".aiview")
	return &AuthStore{
		dir:  dir,
		file: filepath.Join(dir, "xiaohongshu_credential.json"),
	}
}
