...
// AuthStore manages Kuaishou cookie persistence.
type AuthStore struct {
	dir  string
	file string
}

// credential holds Kuaishou authentication data on disk.
type credential struct {
	Cookie string `json:"cookie"`
}

// NewAuthStore creates a new auth store for Kuaishou.
func NewAuthStore() *AuthStore {
	home, _ := os.UserHomeDir()
	dir := filepath.Join(home, ".aiview")
	return &AuthStore{
		dir:  dir,
		file: filepath.Join(dir, "kuaishou_credential.json"),
	}
}
