...
// AuthStore manages Weibo cookie persistence.
type AuthStore struct {
	dir  string
	file string
}

// credential holds Weibo authentication data on disk.
type credential struct {
	Cookie string `json:"cookie"`
}

// NewAuthStore creates a new auth store for Weibo.
func NewAuthStore() *AuthStore {
	home, _ := os.UserHomeDir()
	dir := filepath.Join(home, ".aiview")
	return &AuthStore{
		dir:  dir,
		file: filepath.Join(dir, "weibo_credential.json"),
	}
}
