  ...
  aiview douyin login --auto`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if auto {
				...
				cookies, err := browser.GetCookies("https://www.douyin.com", 2*time.Minute)
				if err != nil {
			...
			if cookie == "" {
				return aiverr.NotAuthenticated("douyin", "cookie is required, use --cookie flag or --auto")
			}
