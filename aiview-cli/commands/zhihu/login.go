...
// NewLoginCmd creates the login command for Zhihu.
func NewLoginCmd(saveCookie func(cookie string) error, getClient func() Client) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "login",
		Short: "Login to Zhihu with cookie",
		Long: `Login to Zhihu using a browser cookie.

Examples:
  aiview zhihu login --cookie "your_cookie_here"
  aiview zhihu login --auto`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if auto {
				...
				cookies, err := browser.GetCookies("https://www.zhihu.com", 2*time.Minute)
				if err != nil {
			...
			if cookie == "" {
				return aiverr.NotAuthenticated("zhihu", "cookie is required, use --cookie flag or --auto")
			}
	...
	cmd.Flags().StringVar(&cookie, "cookie", "", "Zhihu browser cookie")
