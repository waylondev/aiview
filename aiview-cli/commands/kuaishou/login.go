...
// NewLoginCmd creates the login command for Kuaishou.
func NewLoginCmd(saveCookie func(cookie string) error, getClient func() Client) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "login",
		Short: "Login to Kuaishou with cookie",
		Long: `Login to Kuaishou using a browser cookie.

Examples:
  aiview kuaishou login --cookie "your_cookie_here"
  aiview kuaishou login --auto`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if auto {
				...
				cookies, err := browser.GetCookies("https://www.kuaishou.com", 2*time.Minute)
				if err != nil {
	...
	cmd.Flags().StringVar(&cookie, "cookie", "", "Kuaishou browser cookie")
